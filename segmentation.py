...
def aug_img1(orig_arr, mask_arr):
    transform = A.Compose(
        [
            # 对比度亮度拉伸
...
def aug_img2(orig_arr, mask_arr):
    transform = A.Compose(
        [
            # 对比度亮度拉伸
...
def main():
    root = Path(r'E:\data\ricedata_v3.0\ori')
    root_img = root / "JPEGImages"
    root_mask = root / "SegmentationClass"
    save_img_mask = root / "save"
    mask_mid = 'SegmentationClass'
    orig_mid = 'JPEGImages'
    if not (save_img_mask / orig_mid).exists():
        (save_img_mask / orig_mid).mkdir(parents=True)
    if not (save_img_mask / mask_mid).exists():
        (save_img_mask / mask_mid).mkdir(parents=True)
...
# 数据增强
            # aug_orig, aug_mask = aug_img1(arr_orig, arr_mask)
            # img_orig = Image.fromarray(aug_orig)
            # img_mask = Image.fromarray(aug_mask)
            # img_mask.putpalette(mask_palette)
            # save_img(img_orig, str(save_img_mask / orig_mid / (path_orig.stem + '_%s.jpg')))
            # save_img(img_mask, str(save_img_mask / mask_mid / (path_mask.stem + '_%s.png')))

            # aug_orig, aug_mask = aug_img2(arr_orig, arr_mask)
            # img_orig = Image.fromarray(aug_orig)
            # img_mask = Image.fromarray(aug_mask)
            # img_mask.putpalette(mask_palette)
            # save_img(img_orig, str(save_img_mask / orig_mid / (path_orig.stem + '_%s.jpg')))
