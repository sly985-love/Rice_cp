...
def Small_Scale_Jittering_3(mask, img, min_scale=0.8, max_scale=1.0):
...
# 3通道的mask
# ------对mask和对应的原图进行尺度缩放，再对缩放后图像进行填充（缩小了）或裁剪（放大了），保证输出还是原来的大小------# Middle_Scale_Jittering_3
...
def new_copy_paste(mask_src, img_src, mask_dst, img_dst):
    ...
    mask, img = random_flip_horizontal(mask, img)
    return mask, img
...
def save_img(image, path_pattern):
    ...
    # cv2.imwrite(path_pattern % i) data_new_all/BrownSpot_Pure_139/ pic_data/BrownSpot/BrownSpot95_10
    # brownSpot_40  pic_data/BacterialBlight/bacterialblight_50


def main():
    root = Path("xyp")
    # os.mkdir(root / "BS96_10" / "JPEGImages")
    # os.mkdir(root / "BS96_10" / "SegmentationClass")
    root_img = root / "BrownSpot96" / "JPEGImages"
    root_mask = root / "BrownSpot96" / "SegmentationClass"
    save_img = root / "BS96_10" / "JPEGImages"
    save_mask = root / "BS96_10" / "SegmentationClass"
    ...
    for name in orig_list:
        if name in mask_list:
            ...
            mask_dst, img_dst = Small_Scale_Jittering_3(mask_dst, img_dst)  # 做尺度缩放、裁剪或填充，保证输出和原来一样大小
            M = 10
            for i in range(8,10):
