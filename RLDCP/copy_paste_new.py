...
def main():
    ...
    save_img = root / "new_copy_paste_save"/ "JPEGImages"
    save_mask = root / "new_copy_paste_save" / "SegmentationClass"

    orig_list = [path.stem for path in root_img.iterdir()]
    mask_list = [path.stem for path in root_mask.iterdir()]

    # mask_dst_path_ = np.random.choice(mask_list)  # 随机选取mask里面一张 n3_99.png
    # mask_dst_path = root_mask / (mask_dst_path_ + '.png')
    # img_dst_path = root_img / (mask_dst_path_ + '.jpg')

    # print(mask_dst_path,img_dst_path)
    ...
    for name in orig_list:
        if name in mask_list:
            ...
            # mask_src = Image.open(path_orig)
            # img_src = Image.open(path_mask)
            # mask_dst = Image.open(mask_dst_path)
            ...
            if str(path_orig) == str(mask_dst_path):
                ...
                # print(path_orig, path_mask, mask_dst_path, img_dst_path)

                # new_mask, new_img = new_copy_paste(mask_src, img_src, mask_dst, img_dst)
                # cv2.imwrite(str(save_img_mask / (path_orig.stem + '_.jpg')), new_img)
                # cv2.imwrite(str(save_img_mask / (path_orig.stem + '_.png')), new_mask)
